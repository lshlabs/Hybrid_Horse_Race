...
export function HorseSelectionPage() {
  ...
  const currentPlayer = useMemo(() => {
    if (!room || !userId) {
      console.log('[HorseSelectionPage] currentPlayer: room or userId missing', {
        room: !!room,
        userId,
      })
      return null
    ...
    if (playerId) {
      ...
      console.log(
        '[HorseSelectionPage] Found player by playerId:',
        found?.id,
        'searching for:',
        playerId,
      )
      ...
      if (!playerExists) {
        ...
        const checkInterval = setInterval(() => {
          ...
          if (retryCount >= maxRetries) {
            ...
            console.error(
              '[HorseSelectionPage] Player not found after retries, redirecting to lobby',
              {
                playerId,
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : '주행 습성 선택에 실패했습니다.'
        ...
        <div className="text-center">
          ...
          {playerNotFound && (
            <div className="mt-4 space-y-2">
              <p className="text-sm text-neutral-400">플레이어 정보를 불러오는 중...</p>
              <p className="text-xs text-neutral-500">roomId: {roomId}</p>
              <p className="text-xs text-neutral-500">playerId: {playerId || 'none'}</p>
              <p className="text-xs text-neutral-500">players: {players.length}명</p>
              {players.length > 0 && (
      ...
      <div className="w-full max-w-4xl rounded-3xl border border-white/10 bg-surface/80 p-8 shadow-surface backdrop-blur-lg">
        ...
        {currentPlayer.horseStats && (
          <div className="mb-6 rounded-xl border border-white/10 bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.35em] text-neutral-400 mb-2">말 능력치</p>
            <div className="grid grid-cols-4 gap-2 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 text-neutral-100">
                  {currentPlayer.horseStats.jockeySkill}
                </span>
              </div>
            </div>
          </div>
