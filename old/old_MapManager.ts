...
export default class MapManager {
  ...
  private trackStartWorldXPx!: number
  private trackFinishWorldXPx!: number
  ...
  private createStartFinishStripes(
    ...
    this.trackStartWorldXPx = startMarginValue

    if (raceDistance !== undefined) {
      this.trackLengthPx = raceDistance
    } else {
      const finishMarginValue = finishMargin !== undefined ? finishMargin : defaultMargin
      this.trackLengthPx = this.totalMapWidth - startMarginValue - finishMarginValue
    }

    this.trackFinishWorldXPx = this.trackStartWorldXPx + this.trackLengthPx

    // 깃발 배치
    this.startStripe = this.scene.add
      .image(this.trackStartWorldXPx, stripeY, 'finishStripe')
      .setOrigin(0.5, 1)
      .setDepth(5)

    this.finishStripe = this.scene.add
      .image(this.trackFinishWorldXPx, stripeY, 'finishStripe')
      .setOrigin(0.5, 1)
      .setDepth(5)
  }
   ...
   */
  updateStripePositions(cameraScrollPx: number) {
    if (this.startStripe) {
      this.startStripe.x = this.trackStartWorldXPx - cameraScrollPx
    }

    if (this.finishStripe) {
      this.finishStripe.x = this.trackFinishWorldXPx - cameraScrollPx
    }
  }
}
